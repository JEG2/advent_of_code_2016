defmodule Descrambler do
  ...
  def execute([instruction | instructions], data) do
    execute(instructions, unmodify(data, instruction))
  end

  def unmodify(data, {:swap_positions, x, y}) do
    modify(data, {:swap_positions, x, y})
  end
  def unmodify(data, {:swap_letters, x, y}) do
    modify(data, {:swap_letters, x, y})
  end
  def unmodify(data, {:rotate, :left, x}) do
    modify(data, {:rotate, :right, x})
  end
  def unmodify(data, {:rotate, :right, x}) do
    modify(data, {:rotate, :left, x})
  end
  def unmodify(data, {:rotate_position, x}) do
    find_rotation(1, data, x)
  end
  def unmodify(data, {:reverse, x, y}) do
    modify(data, {:reverse, x, y})
  end
  def unmodify(data, {:move, x, y}) do
    modify(data, {:move, y, x})
  end

  def find_rotation(i, data, x) do
    start = modify(data, {:rotate, :left, i})
    if modify(start, {:rotate_position, x}) == data do
      start
    else
      find_rotation(i + 1, data, x)
    end
  end
